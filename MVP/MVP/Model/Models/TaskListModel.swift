...
extension TaskListModel: EntityModelMapProtocol{
    ...
    static func mapFromEntity(_ entity: TasksList) -> Self {
        guard let tasksListed = entity.tasks else {
            return .init(title: entity.title, icon: entity.icon, tasks: [TaskModel](), createdAt: Date())
        }
        ...
        return .init(id: entity.id, title: entity.title, icon: entity.icon, tasks: tasks, createdAt: entity.createdAt)
